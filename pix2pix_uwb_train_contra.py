...
parser.add_argument("--model_name", type=str, default='pix2pix_uwb_contra', help="name of model")
...
parser.add_argument("--mode", type=str, default='eval_uwb', help="test train eval_uwb")
...
app = ''
train_dataset_file = 'data/train_uwb%s.npy' % (app)
test_dataset_file = 'data/test_uwb%s.npy' % (app)
    ...
    for epoch in range(0, opt.epochs):
        for i, data in enumerate(train_dataloader):
            ...
            for k in range(2):
                ...
                # # contrasive loss
                contra_loss = MSECriterion(h1, h2) * 0.1
                # contra_loss=0
...
def test_main(model_path):
    ...
    np.save('images/temp/cos_scores_our%s.npy'% (app), cos_scores)
    np.save('images/temp/rmse_scores_our%s.npy'% (app), rmse_scores)
    # np.save('images/temp/cc_scores_our%s.npy'% (app), cc_scores)

    print('start to evaluate_ecg_interval')
    evaluate_ecg_interval('images/%s/test/npy/' % (opt.model_name))
    print('end to evaluate_ecg_interval')


def eval_uwb(model_path, data_path):
    print('start to eval')
    ...
    for index in range(file_ecg.shape[0]):
        ...
        file_amp_data[index, :] = (file_amp_data[index, :] - np.mean(file_amp_data[index, :])) / np.std(
            file_amp_data[index, :])
        file_pha_data[index, :] = (file_pha_data[index, :] - np.mean(file_pha_data[index, :])) / np.std(
            file_pha_data[index, :])

        # map to [-1, 1]
        ecg = mapminmax(file_ecg[index, :])
        amplitude = mapminmax(file_amp_data[index, :])
        phase = mapminmax(file_pha_data[index, :])
        ...
        _, generated_ECG, _, _ = G(amplitude, phase, amplitude, phase)
        generated_ECG = generated_ECG[0, 0, :].cpu().detach().numpy()
        ...
        plt.title('generated_ECG')
        plt.subplot(4, 1, 3)
        plt.plot(time_x, amplitude[0, 0, :].cpu())
        plt.title('amplitude_data')
        plt.subplot(4, 1, 4)
        plt.plot(time_x, phase[0, 0, :].cpu())
        plt.title('phase_data')
        fig.tight_layout()

        plt.savefig("images/temp/%d.png" % (index))
        plt.close()
...
if __name__ == '__main__':
    model_path = 'weights/saved/pix2pix_uwb_contra_97.pth'
    if opt.mode == 'train':
        train_main()
    elif opt.mode == 'test':
        test_main(model_path)
    elif opt.mode == 'eval_uwb':
        data_path = '/home/wz/桌面/UWB2ECG/data/uwb_data/patient_data/心率过速/1.mat'
        eval_uwb(model_path, data_path)