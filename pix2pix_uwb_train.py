...
parser.add_argument("--model_name", type=str, default='pix2pix_uwb', help="name of model")
...
if opt.mode == 'train':
    ...
    train_data = np.load('data/train_uwb.npy')
    test_data = np.load('data/test_uwb.npy')
...
def train_main():
    ...
    for epoch in range(0, opt.epochs):
        ...
        if epoch % 2 == 1:
            torch.save(G.state_dict(), "weights/pix2pix_uwb_%d.pth"%(epoch))
...
def test_main(model_path):
    ...
    np.save('images/temp/cos_scores_our.npy', cos_scores)
    np.save('images/temp/rmse_scores_our.npy', rmse_scores)
    np.save('images/temp/cc_scores_our.npy', cc_scores)

    print('start to evaluate_ecg_interval')
    evaluate_ecg_interval('images/pix2pix_uwb/test/npy/')
...
if __name__ == '__main__':
    model_path = 'weights/saved/pix2pix_uwb_0601edit4_99_new.pth'
    if opt.mode == 'train':
        train_main()
    elif opt.mode == 'test':
